context('Checkbox', () => {
  ...
  describe(`Group`, () => {
    ...
    it('Choose an option', () => {
      ...
      cy.get('.text--p').invoke('text').should('eq', 'Values selected: ["cat"]')

      cy.get('.checkbox:nth-of-type(2)').click()
      cy.get('.text--p').invoke('text').should('eq', 'Values selected: ["cat","dog"]')

      cy.get('.checkbox:nth-of-type(3)').click()
      cy.get('.text--p').invoke('text').should('eq', 'Values selected: ["cat","dog","horse"]')
    })
  })
})
