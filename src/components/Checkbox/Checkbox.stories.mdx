  ...
  <Story name='CustomLabel'>
    {() => {
      ...
      return (
        <div>
          <Text>Values selected: {JSON.stringify(values)}</Text>
          <Checkbox name={'checkbox'} value={values.includes('cat')} onChange={(value) => handleChange('cat', value)}><div>Cat</div></Checkbox>
          <Checkbox name={'checkbox'} value={values.includes('dog')} onChange={(value) => handleChange('dog', value)}><div>Dog</div></Checkbox>
          <Checkbox name={'checkbox'} value={values.includes('horse')} onChange={(value) => handleChange('horse', value)}><div>Horse</div></Checkbox>
        </div>
      )
    }}
  </Story>
