  ...
  <Story name='Searchable'>
    {() => {
      ...
      const options = [...Array(1000).keys()].map((item, index) => {
        if (index % 2) {
          return {label: `Option ${item}`, value: `option-${item}`}
        } else {
          return {label: `Option ${item} - Sed a ultrices orci. Integer tincidunt justo sit amet libero egestas, eget pharetra ligula egestas. Sed laoreet, velit non scelerisque interdum, tellus nisi maximus sem, congue fringilla sapien justo sed mi..`, value: `option-${item}`}
        }
      })
      return (
        <div>
